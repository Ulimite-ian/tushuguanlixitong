...
def dohuanshu(request):
    ...
    print("record is: ", r)
    print("record0 is: ", r[0])
    print("recordState is: ", r[0].State)
    print("recordReturnTime is: ", r[0].ReturnTime)
    print("datetime.datetime.today is: ", datetime.datetime.today())
    print("r[0].ReturnTime tzinfo is: ", r[0].ReturnTime.tzinfo )
    print("datetime.datetime.today() tzinfo is: ", datetime.datetime.today().tzinfo)
    if len(r)==0 or r[0].State==1:
        array = {
            'msg':"没有借过此书 或者已经还书成功"
        }
    else:
        ...
        b.save()
        r[0].State = 1
        if r[0].ReturnTime.replace(tzinfo=None)<datetime.datetime.today():
            array = {
                'msg': "还书过晚"
            }
        else:
